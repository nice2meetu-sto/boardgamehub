...
create or replace function public.save_rating(
  p_player_id text, p_pin text, p_game_id text, p_rating numeric, p_memo text)
returns json
language plpgsql security definer
set search_path = public, extensions
as $$
...
begin
  ...
  insert into public.ratings(player_id, game_id, rating, memo, updated_at)
  values (p_player_id, p_game_id, p_rating, coalesce(p_memo, ''), v_now)
  on conflict (player_id, game_id) do update
    set rating = excluded.rating, memo = excluded.memo, updated_at = excluded.updated_at;

  return json_build_object('player_id', p_player_id, 'game_id', p_game_id,
                           'rating', p_rating, 'memo', coalesce(p_memo,''), 'updated_at', v_now);
end $$;
