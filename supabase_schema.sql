...
-- 플레이 세션 수정. 입력자 본인 또는 관리자만.
--   payload에 'participants' 배열이 오면 → 참가자 전체 교체(이름·게스트↔회원·
--     점수·승패 편집, 게스트→회원 연동 포함). 세션·게임·작성자는 보존.
--   'participants'가 없으면 → 레거시(날짜·시간 + 'rows'의 점수/승패만) 갱신.
